...
} from "../../features/girlfriends/girlFriendsApi";
import SearchGirlFriend from "../../components/pages/girlfriends/SearchGirlFriend";
import { useNavigate } from "react-router-dom";

function GirlFriends() {
  const navigate = useNavigate();
  const handleNavigate = (data) => {
    navigate("/updategirlfriend", {
      state: { payload: data },
    });
  };

  const { data, isLoading, isError } = useGetAllGirlFriendsQuery();
  const [
    deleteGirlFriend,
    { data: deletedData, isLoading: deletionLoading, isError: deletionError },
  ] = useDeleteGirlFriendMutation();
      ...
      return (
        ...
        >
          ...
          <div className="px-3 py-6 font-rubik">
            <h2 className="text-lg font-medium ">Name: {item?.name}</h2>
            <p className="text-fade">Age: {item?.age}</p>
            {/* {item?.pros.map((pro) => {
              <p className="text-fade">Age: {pro}</p>;
            })} */}
            <p className="text-fade">Pros: {item?.pros}</p>
            <p className="text-fade">Cons: {item?.cons}</p>
            <p className="text-fade">Type: {item?.type}</p>
            <p className="text-fade">Status: {item?.status}</p>
          </div>

          {/* delete button  */}
          <button
            onClick={() => deleteGirlFriend(item?._id)}
            className="w-7 h-7 flex items-center justify-center bg-errorColor rounded-full absolute top-2 right-2"
          >
            <svg
          ...
          <button
            ...
            className="w-7 h-7 flex items-center justify-center bg-errorColor rounded-full absolute top-2 left-2"
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
              width="20"
